...
def tactical_features_for_representation(
    observation: NeuralObservation,
    action: ActionRepresentation,
    representation,
) -> tuple[float, ...]:
    """Compute V6 tactical features for one masked, action-conditioned card."""
    ...
    requirements = _active_requirements(observation, representation)
    union_required, echo_required, domination_required = requirements
    union_active = union_required and _union_active(observation, action)
    echo_active = echo_required and _echo_active(observation)
    missing = _domination_missing_count(observation, action)
...
class StructuredSemanticV6TacticalActionScorer(StructuredSemanticV5DeckStateScorer):
    ...
    def _features_for_action(
        ...
        if action.action_type != "play_card" or action.card_definition_id is None:
            return (0.0,) * _TACTICAL_FEATURE_SIZE
        return tactical_features_for_representation(
            observation,
            action,
            self._semantic_cache.get(action.card_definition_id),
        )
